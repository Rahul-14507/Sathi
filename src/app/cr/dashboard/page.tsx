...
export default function CRDashboard() {
  ...
  const [remindConfirmTask, setRemindConfirmTask] = useState<any>(null);
  ...
  const handleRemind = async () => {
    if (!remindConfirmTask) return;
    const task = remindConfirmTask;
    setRemindingTaskId(task.id);
    setRemindConfirmTask(null);
    try {
      ...
      console.error(err);
      toast.error("Error sending reminder blast.");
    } finally {
  ...
  const handlePostTask = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDescription("");
        setDeadline("");
        toast.success("Global Academic Task Posted Successfully!");
        fetchTasks();
      } else {
        toast.error(
          "Failed to post task. Are your Cosmos DB details configured?",
        );
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <>
      <main className="min-h-screen bg-slate-950 text-slate-100 p-8 dark">
        <div className="max-w-3xl mx-auto space-y-8">
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl mt-8">
            <h2 className="text-xl font-semibold mb-6 flex items-center gap-2">
              <Users className="w-5 h-5 text-emerald-400" /> Currently Assigned
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-2">
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => setRemindConfirmTask(task)}
        ...
        <AlertDialogContent className="bg-slate-900 border border-slate-700">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-slate-400">
              ...
              {deleteConfirmTask?.title}"? This will remove the task for all
              students in Section {sectionId}. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog
        open={!!remindConfirmTask}
        onOpenChange={(open) => !open && setRemindConfirmTask(null)}
      >
        <AlertDialogContent className="bg-slate-900 border border-slate-700">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-white">
            ...
            <AlertDialogDescription className="text-slate-400">
              Are you sure you want to send an email reminder to all enrolled
              students in Section {sectionId} for the task "
              {remindConfirmTask?.title}"?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="bg-transparent text-white border-slate-700 hover:bg-slate-800">
